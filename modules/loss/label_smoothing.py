...
class LabelSmoothing(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        one_hot_labels.scatter_(1, targets.long().view(-1, 1), 1)
        qk = (1 - self.eps) * one_hot_labels + self.eps/self.nClasses
        loss = self.logsoftmax(inputs) * qk

        return -loss.sum(1).mean()

#Implementation from: https://github.com/KaiyangZhou/deep-person-reid/blob/master/torchreid/losses/cross_entropy_loss.py
